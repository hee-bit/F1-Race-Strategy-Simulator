...
def main():
    ...
    with main_left:
        ...
        st.markdown("* **실시간 데이터 동기화**: 좌측 사이드바 제어창에서 선택된 옵션들은 우측 모니터링 보드와 실시간 연동됩니다.\n* **몬테카를로 시뮬레이션 알고리즘**: FastF1 실데이터 모델링을 기반으로 수백 가지 레이스 시나리오를 예측 연산합니다.")
        
        st.markdown("---")
        
        st.markdown('<div class="section-label">⚙️ 레이스 컨트롤 전략 보조 가이드</div>', unsafe_allow_html=True)
        st.markdown("* **트랙 성향 인자 자동 연산**: 서킷별 DRS 효율, Dirty Air 영향성 및 교통(Traffic) 정체 패널티가 상시 반영 중입니다.\n* **실시간 연산 준비**: 입력 데이터를 확인하신 후 좌측 사이드바 하단의 트리거 버튼을 눌러 시뮬레이션을 개시하세요.")
        
        st.markdown("---")

        # 2. 타이어 열화율 & 피트 손실 추정치 (가로 배치)
        col_tyre, col_pit = st.columns([1, 1])

        with col_tyre:
            st.markdown('<div class="section-label">타이어 열화율</div>', unsafe_allow_html=True)
            st.caption("주행할수록 늘어나는 랩타임 페널티(초)")
            tyre_table = [[t, i['base_offset'], i['deg_per_lap'], i['recommended_stint']] for t, i in tyre_model.items()]
            st.dataframe(pd.DataFrame(tyre_table, columns=['타이어', '성능차(초)', '열화율', '스틴트']), use_container_width=True, hide_index=True)

        with col_pit:
            st.markdown('<div class="section-label">피트 레인 손실 추정치</div>', unsafe_allow_html=True)
            st.caption("피트 레인 통과 시 손해 보는 총 시간입니다.")
            c1, c2 = st.columns(2)
            c1.metric("Median (평균)", f"{pit_stats['median_pit_loss']} 초")
            c2.metric("Max (최대)", f"{pit_stats['recommended_max_pit_loss']} 초")
    ...
    if start_calc:
        ...
        with res_right:
            ...
            st.success(f"최적 피트 랩: **{best['pit_laps']}** / 다음 타이어: **{best['next_tyres']}**")

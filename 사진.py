...
def main():
    ...
    with main_left:
        ...
        st.markdown('<div style="color: #98a2b3; margin-bottom: 10px;">• 주행할수록 타이어가 닳아 한 바퀴를 도는 데 시간이 얼마나 더 걸리는지(초) 나타낸 열화 모델입니다.</div>', unsafe_allow_html=True)
        ...
        # 2. 피트 레인 손실 섹션 (열화율 바로 아래로 이동)
        st.markdown('<div class="section-label">피트 레인 손실 추정치</div>', unsafe_allow_html=True)
        st.markdown('<div style="color: #98a2b3; margin-bottom: 10px;">• 경주용 차가 새로운 타이어로 갈아끼우기 위해 피트 레인을 통과할 때 손해 보는 총 시간입니다.</div>', unsafe_allow_html=True)
        
        # 메트릭 박스 배치
        col1, col2 = st.columns(2)
        col1.metric("Median Pit Loss", f"{pit_stats['median_pit_loss']} 초")
        col2.metric("Recommended Max", f"{pit_stats['recommended_max_pit_loss']} 초")
        
        st.markdown("<div style='margin: 25px 0; border-top: 1px solid rgba(255,255,255,0.08);'></div>", unsafe_allow_html=True)
    ...
    if start_calc:
        adjusted_pit_loss = adjust_pit_loss_for_track_status(pit_stats['median_pit_loss'], safety_mode)

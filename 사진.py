...
def main():
    ...
    # 1. 사이드바 및 왼쪽 패널 (입력창과 차트)
    main_left, main_right = st.columns([0.8, 1.2])
    ...
    with main_left:
        ...
        if st.sidebar.radio("피트 손실시간 계산", ["자동", "수동"]) == "수동":
            green_pit_loss = st.sidebar.number_input("손실시간 입력(초)", value=22.0)
        ...
        m1, m2 = st.columns(2)
        m1.metric("Median Loss", f"{pit_stats['median_pit_loss']}s")
        m2.metric("Rec. Max", f"{pit_stats['recommended_max_pit_loss']}s")

    # 2. 오른쪽 결과 출력 (수직 배치로 겹침 방지)
    with main_right:
        st.markdown(f"<h2>🏎️ 서킷: {track_name}</h2>", unsafe_allow_html=True)
        path = TRACK_IMAGES_PATHS.get(track_name)
        if path and path.exists(): st.image(str(path), width=600)
        ...
        if start_calc:
            ...
            if not result_df.empty:
                ...
                st.dataframe(recommend_stop_count(result_df)['summary_table'], use_container_width=True, hide_index=True)
                ...
                st.success(f"추천 피트 랩: **{best['pit_laps']}** (타이어: {best['next_tyres']})")
                
                # 메트릭을 가로로 3개 배치 (수직 배치 원하시면 st.columns 제거하고 그냥 적으세요)

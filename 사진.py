...
def get_track_params(track_name):
    return TRACK_PARAMS.get(track_name, {"overtake_factor": 1.0, "drs_factor": 1.0, "dirty_air_factor": 1.0, "traffic_factor": 1.0})
...
def recommend_tyre_change_time(front_gap, rear_gap, safety_mode, current_position):
    ...
    if min_gap <= 1.0: target -= 0.2
    elif min_gap <= 2.0: target -= 0.1
    elif min_gap >= 5.0: target += 0.1
    if safety_mode == "SC": target += 0.15
    elif safety_mode == "VSC": target += 0.10
    target = max(1.8, min(target, 2.6))
    comment = ("기록급 피트스탑 필요" if target <= 1.9 else "매우 빠른 타이어 교체 필요" if target <= 2.1 else "상위권 유지 가능: 빠른 피트 작업 필요" if target <= 2.3 else "여유는 있지만 더 빠를수록 유리")
    return {"baseline_tyre_change_time": 2.2, "recommended_max_tyre_change_time": round(target, 2), "comment": comment}
...
def build_recent_pace_lookup(clean_laps_df, track_name, current_lap, lookback=RECENT_LAPS_FOR_PACE):
    ...
    if df.empty: return {}
    return {drv: float(grp.sort_values("LapNumber")["LapTimeSeconds"].tail(lookback).median()) for drv, grp in df.groupby("Driver") if len(grp) >= 2}
...
def get_effective_pace_offset(driver, driver_pace_model, recent_pace_lookup, base_lap):
    ...
    if driver in recent_pace_lookup: return 0.45 * long_term + 0.55 * (recent_pace_lookup[driver] - base_lap)
    return long_term
...
def generate_strategy_candidates(total_laps, current_lap, tyre_model, current_tyre_life, allow_zero_stop=ALLOW_ZERO_STOP_DEFAULT):
    ...
    if current_tyre_life < FORCE_ONE_STOP_IF_TYRE_LIFE_AT_LEAST and allow_zero: candidates.append([])
...
def build_rival_states_from_reference(raw_laps_df, track_name, current_lap, my_driver, driver_pace_model, recent_pace_lookup, base_lap, tyre_model, total_laps):
    df = raw_laps_df[(raw_laps_df["GrandPrix"].str.lower() == track_name.lower()) & (raw_laps_df["LapNumber"] == current_lap - 1)].dropna(subset=["Driver", "Position", "Time"]).sort_values("Position")
    ...
    for _, row in df[df["Driver"] != my_driver].head(MAX_RIVALS).iterrows():
        ...
        try: tyre_life = int(float(row["TyreLife"])) if pd.notna(row.get("TyreLife")) else 8
        except Exception: tyre_life = 8
        ...
        rivals.append({"driver": row["Driver"], "race_time": float(race_time), "position": int(row["Position"]), "compound": comp, "tyre_life": tyre_life, "laps_since_stop": tyre_life, "pace_offset": pace_offset, "front_gap": 99.0, "rear_gap": 99.0, "strategy": rival_strat, "strategy_index": 0})
...
def build_fallback_rival_states(selected_rivals, driver_pace_model, recent_pace_lookup, base_lap, rng):
    ...
    for i, drv in enumerate(selected_rivals):
        ...
        rivals.append({"driver": drv, "race_time": float(rng.uniform(0.5, 6.5)), "position": i + 1, "compound": ["MEDIUM", "HARD", "SOFT"][i % 3], "tyre_life": int(rng.integers(4, 16)), "laps_since_stop": int(rng.integers(4, 16)), "pace_offset": pace_offset, "front_gap": float(rng.uniform(0.7, 3.5)), "rear_gap": float(rng.uniform(0.7, 3.5)), "strategy": [], "strategy_index": 0})
    return rivals
...
def predict_driver_lap_time(driver, base_lap, pace_offset, compound, tyre_life, tyre_model, front_gap, rear_gap, drs_available, laps_since_stop, rng, track_name, safety_mode="NONE"):
    ...
    if tyre_life >= VERY_OLD_TYRE_THRESHOLD: lap_time += (tyre_life - VERY_OLD_TYRE_THRESHOLD + 1) * OLD_TYRE_EXTRA_PENALTY_PER_LAP
    if safety_mode == "SC": return (lap_time * 1.32) + noise
    if safety_mode == "VSC": return (lap_time * 1.12) + noise
    return (lap_time + traffic_penalty(front_gap, track_name) + dirty_air_penalty(front_gap, track_name) + rear_pressure_penalty(rear_gap) - drs_gain(front_gap, track_name, drs_available) + warmup_penalty(laps_since_stop, compound) - undercut_bonus(laps_since_stop, compound, front_gap, track_name) + noise)

def can_overtake(gap_to_front, lap_advantage, front_gap, track_name, drs_available):
    p = get_track_params(track_name)
    return ((gap_to_front <= OVERTAKE_BASE_GAP * p["overtake_factor"]) and (lap_advantage >= (OVERTAKE_MIN_ADVANTAGE / max(p["overtake_factor"], 0.35)) * (0.88 if drs_available else 1.0)) and (front_gap <= 1.5))
...
def simulate_race_once(total_laps, current_lap, base_lap, tyre_model, my_state, rivals, adjusted_pit_loss, rng, track_name, safety_mode="NONE"):
    ...
    for lap in range(current_lap, total_laps + 1):
        ...
        if sc_rem > 0: sc_rem -= 1
        elif vsc_rem > 0: vsc_rem -= 1
        lap_times = {}
        for car in all_cars:
            just_pitted = False
            if car["strategy_index"] < len(car["strategy"]) and lap == car["strategy"][car["strategy_index"]]["pit_lap"]:
                ...
                r = rng.random()
                if r < 0.02: penalty += rng.uniform(4.0, 7.0)
                elif r < 0.08: penalty += rng.uniform(1.2, 2.5)
            ...
            lt = predict_driver_lap_time(car["driver"], base_lap, car["pace_offset"], car["compound"], car["tyre_life"], tyre_model, car["front_gap"], car.get('rear_gap', 2.0), car["front_gap"] <= 1.0, car["laps_since_stop"], rng, track_name, mode)
            if just_pitted: lt += 0.45 if car["front_gap"] <= 1.5 else 0.20 if car["front_gap"] <= 3.0 else 0.0
            car["race_time"] += lt
            car["tyre_life"] += 1
...
def simulate_many(total_laps, current_lap, base_lap, tyre_model, my_state, rivals, adjusted_pit_loss, track_name, safety_mode="NONE", n=300, seed=42):
    ...
    return {"expected_finish_time": round(float(np.mean(df["finish_time"])), 2), "finish_time_std": round(float(np.std(df["finish_time"])), 4), "expected_position": round(float(np.mean(df["position"])), 2), "most_likely_position": int(df["position"].mode().iloc[0])}

def sort_result_df(result_df):
    return result_df.sort_values(by=["strategy_score", "expected_finish_time", "expected_position", "finish_time_std"], ascending=[True, True, True, True]).reset_index(drop=True)
...
def strategy_to_row(strategy, sim, current_tyre_life):
    ...
    penalty = 0.0
    if len(strategy) == 0: penalty = NO_STOP_TIME_PENALTY + (max(0, current_tyre_life - FORCE_ONE_STOP_IF_TYRE_LIFE_AT_LEAST + 1) * 0.45 if current_tyre_life >= FORCE_ONE_STOP_IF_TYRE_LIFE_AT_LEAST else 0.0)
    return {"stops": len(strategy), "pit_laps": [x["pit_lap"] for x in strategy], "next_tyres": [x["next_tyre"] for x in strategy], "expected_finish_time": sim["expected_finish_time"], "finish_time_std": sim["finish_time_std"], "expected_position": sim["expected_position"], "most_likely_position": sim["most_likely_position"], "strategy_score": round(float(score + penalty), 4), "no_stop_penalty": round(float(penalty), 4)}
...
def build_my_state(my_driver, current_position, current_compound, current_tyre_life, front_gap, rear_gap, driver_pace_model, recent_pace_lookup, base_lap, strategy, my_initial_race_time):
    return {"driver": my_driver, "race_time": my_initial_race_time, "position": current_position, "compound": current_compound, "tyre_life": current_tyre_life, "laps_since_stop": current_tyre_life, "pace_offset": get_effective_pace_offset(my_driver, driver_pace_model, recent_pace_lookup, base_lap), "front_gap": front_gap, "rear_gap": rear_gap, "strategy": strategy, "strategy_index": 0}
...
def simulate_strategy_job(args):
    ...
    my_state = build_my_state(my_driver, current_position, current_compound, current_tyre_life, front_gap, rear_gap, driver_pace_model, recent_pace_lookup, base_lap, strategy, my_initial_race_time)
...
def run_batch_simulations(strategies, total_laps, current_lap, base_lap, tyre_model, current_position, current_compound, current_tyre_life, front_gap, rear_gap, driver_pace_model, recent_pace_lookup, my_driver, rivals, adjusted_pit_loss, track_name, safety_mode, n, seed_base, my_initial_race_time):
    jobs = [(strat, total_laps, current_lap, base_lap, tyre_model, current_position, current_compound, current_tyre_life, front_gap, rear_gap, driver_pace_model, recent_pace_lookup, my_driver, rivals, adjusted_pit_loss, track_name, safety_mode, n, seed_base + idx, my_initial_race_time) for idx, strat in enumerate(strategies)]
    if USE_MULTIPROCESSING and len(jobs) >= 4:
        with Pool(processes=max(1, min(cpu_count() - 1, 6))) as pool: return pool.map(simulate_strategy_job, jobs)
...
def evaluate_strategies(total_laps, current_lap, current_compound, current_position, front_gap, rear_gap, base_lap, tyre_model, adjusted_pit_loss, driver_pace_model, my_driver, track_name, raw_laps_df, clean_laps_df, safety_mode, current_tyre_life):
    ...
    rivals, my_initial_race_time = build_rival_states_from_reference(raw_laps_df, track_name, current_lap, my_driver, driver_pace_model, recent_pace_lookup, base_lap, tyre_model, total_laps)
    if not rivals:
        rival_names = [d for d in driver_pace_model.keys() if d != my_driver][:MAX_RIVALS]
        while len(rival_names) < MAX_RIVALS: rival_names.append(f"RIVAL{len(rival_names)+1}")
    ...
    coarse = run_batch_simulations(candidates, total_laps, current_lap, base_lap, tyre_model, current_position, current_compound, current_tyre_life, front_gap, rear_gap, driver_pace_model, recent_pace_lookup, my_driver, rivals, adjusted_pit_loss, track_name, safety_mode, COARSE_SIM_N, 1000, my_initial_race_time)
    coarse_df = sort_result_df(pd.DataFrame(coarse))
    if coarse_df.empty: return coarse_df
    mid_strats = [[{"pit_lap": l, "next_tyre": t} for l, t in zip(r["pit_laps"], r["next_tyres"])] for _, r in coarse_df.head(min(TOPK_MID, len(coarse_df))).iterrows()]
    mid = run_batch_simulations(mid_strats, total_laps, current_lap, base_lap, tyre_model, current_position, current_compound, current_tyre_life, front_gap, rear_gap, driver_pace_model, recent_pace_lookup, my_driver, rivals, adjusted_pit_loss, track_name, safety_mode, MID_SIM_N, 3000, my_initial_race_time)
    mid_df = sort_result_df(pd.DataFrame(mid))
    final_strats = [[{"pit_lap": l, "next_tyre": t} for l, t in zip(r["pit_laps"], r["next_tyres"])] for _, r in mid_df.head(min(TOPK_FINAL, len(mid_df))).iterrows()]
    final = run_batch_simulations(final_strats, total_laps, current_lap, base_lap, tyre_model, current_position, current_compound, current_tyre_life, front_gap, rear_gap, driver_pace_model, recent_pace_lookup, my_driver, rivals, adjusted_pit_loss, track_name, safety_mode, FINAL_SIM_N, 5000, my_initial_race_time)
    ...
    if not pit_df.empty and not stay_df.empty:
        ...
        if ((best_s["expected_finish_time"] - best_p["expected_finish_time"] >= MIN_TIME_GAIN_TO_PIT) or (best_s["expected_position"] - best_p["expected_position"] >= MIN_POSITION_GAIN_TO_PIT) or (current_tyre_life >= FORCE_ONE_STOP_IF_TYRE_LIFE_AT_LEAST)):
...
def recommend_stop_count(result_df):
    if result_df.empty: return {"best_stop_count": None, "summary_table": pd.DataFrame(), "comment": "전략 데이터가 없습니다."}
    summary = result_df.groupby("stops").agg({"expected_position": "mean", "expected_finish_time": "mean", "finish_time_std": "mean", "strategy_score": "mean"}).reset_index().sort_values(by=["strategy_score", "expected_finish_time", "expected_position"])
    best_cnt = int(summary.iloc[0]["stops"])
    return {"best_stop_count": best_cnt, "summary_table": summary, "comment": "시간 기준으로는 무피트도 가능하지만, 참고용입니다." if best_cnt == 0 else f"시간 기준으로 가장 유리한 전략군은 {best_cnt}회 피트 전략입니다."}

def normalize_track_name(track_name):
    for t in ["Bahrain", "Saudi Arabia", "Australia", "Japan", "Monaco"]:
        if track_name.strip().lower() == t.lower(): return t
    return None
...
def main():
    ...
    with main_left:
        ...
        selected_driver_label = st.sidebar.selectbox("시뮬레이션할 내 드라이버 선택", list(DRIVER_OPTIONS.keys()))
        ...
        * **실시간 연산 준비**: 입력 데이터를 확인하신 후 하단의 주황색 트리거 버튼을 눌러 시뮬레이션을 개시하세요.
        ...
        tyre_table = [[tyre, info['base_offset'], info['deg_per_lap'], info['recommended_stint']] for tyre, info in tyre_model.items()]
        st.dataframe(pd.DataFrame(tyre_table, columns=['타이어', '기본 성능차(초)', '랩당 열화율', '권장 스틴트(랩)']), use_container_width=True, hide_index=True)
    ...
    with main_right:
        ...
        with top_left:
            st.markdown(f'<div style="font-size:30px; font-weight:800; color:white; margin-top:40px; padding-bottom:6px; line-height:1.3; margin-bottom:10px;">🏎️ 현재 선택된 서킷: {track_name}</div>', unsafe_allow_html=True)
        ...
        if track_data:
            st.markdown('<div style="background: rgba(20,26,34,0.92); padding:12px 12px 16px 12px; border-radius:24px; border:1px solid rgba(255,255,255,0.08); box-shadow:0 10px 30px rgba(0,0,0,0.25); margin-bottom:20px;">', unsafe_allow_html=True)
    ...
    if start_calc:
        ...
        st.markdown("---")
        st.markdown('<div style="font-size:26px; font-weight:800; color:#ff4d4f; margin-bottom:15px;">📊 시뮬레이션 예측 분석 결과</div>', unsafe_allow_html=True)
        ...
        with res_right:
            ...
            * **데이터상 추천되는 피트 횟수**: 약 {stop_count_info['best_stop_count']}회입니다.
            ...
            if best['stops'] == 0:
                ...
                st.success(f"💡 **추천:** 현재 상황에서는 타이어 교체를 최대 **{tyre_change_info['recommended_max_tyre_change_time']}초** 이내에 끝내고, **{best['pit_laps']}랩**에 피트하는 전략이 가장 유리합니다.")

...
def main():
    ...
    with main_left:
        ...
        current_tyre_life_manual = st.sidebar.number_input("현재 타이어 사용 랩 수", min_value=0, max_value=60, value=12)
        current_position = st.sidebar.number_input("현재 순위", min_value=1, max_value=20, value=3)
        front_gap = st.sidebar.number_input("앞차 간격(초)", min_value=0.0, max_value=60.0, value=1.2, step=0.1)
        rear_gap = st.sidebar.number_input("뒷차 간격(초)", min_value=0.0, max_value=60.0, value=2.5, step=0.1)
        safety_mode = st.sidebar.selectbox("세이프티카", ["NONE", "SC", "VSC"])
        
        start_calc = st.sidebar.button("시뮬레이션 실행")

        st.markdown('<div class="hero-card"><div class="hero-title">F1 Race Strategy Simulator</div></div>', unsafe_allow_html=True)
        
        col_tyre, col_pit = st.columns([1.3, 1.7])
        with col_tyre:
            st.markdown('<div class="section-label">타이어 열화율</div>', unsafe_allow_html=True)
            st.dataframe(pd.DataFrame([[t, i['base_offset'], i['deg_per_lap'], i['recommended_stint']] for t, i in tyre_model.items()], 
                         columns=['타이어', '성능차', '열화율', '스틴트']), use_container_width=True, hide_index=True)
        ...
        with col_pit:
            ...
            st.markdown("""<div style="font-size: 0.9rem; color: #98a2b3; margin-bottom: 10px;">• 피트 통과 시 손해 시간입니다.</div>""", unsafe_allow_html=True)
            st.markdown(f"""
                <div style="display: flex; gap: 10px;">
                    <div style="background: rgba(20, 26, 34, 0.92); border-radius: 18px; padding: 14px; width: 140px;">
                        <div style="font-size: 0.8rem; color: #98a2b3;">Median Pit Loss</div>
                        <div style="font-size: 1.12rem; font-weight: 800;">{pit_stats['median_pit_loss']} 초</div>
                    </div>
                    <div style="background: rgba(20, 26, 34, 0.92); border-radius: 18px; padding: 14px; width: 140px;">
                        <div style="font-size: 0.8rem; color: #98a2b3;">Recommended Max</div>
                        <div style="font-size: 1.12rem; font-weight: 800;">{pit_stats['recommended_max_pit_loss']} 초</div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
    ...
    with main_right:
        st.markdown(f"<h2>🏎️ 서킷: {track_name}</h2>", unsafe_allow_html=True)
        path = TRACK_IMAGES_PATHS.get(track_name)
        if path and path.exists(): st.image(str(path))

    if start_calc:
        # (결과 계산 및 출력 로직은 그대로 사용)
        result_df = evaluate_strategies(total_laps, current_lap, current_compound, current_position, front_gap, rear_gap, base_lap, tyre_model, pit_stats['median_pit_loss'], driver_pace_model, my_driver, track_name, raw_laps_df, clean_laps_df, safety_mode, estimate_current_tyre_life(current_compound, tyre_model))

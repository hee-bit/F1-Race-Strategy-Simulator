...
def main():
    ...
    # [수정] 좌우 비율을 조정하여 왼쪽의 압박감을 줄이고 전체 레이아웃을 쾌적하게 확보
    ...
    with main_left:
        ...
        st.markdown("""<ul style="padding-left: 20px; font-size: 0.85rem; color: #98a2b3;"><li>실시간 연산 준비: 입력 확인 후 버튼을 누르세요.</li></ul>""", unsafe_allow_html=True)
        
        st.markdown("<div style='margin: 20px 0;'></div>", unsafe_allow_html=True)
        
        # [수정] 타이어 정보와 피트 정보의 가독성 확보
        ...
        p1, p2 = st.columns(2)
        p1.metric("Median Loss", f"{pit_stats['median_pit_loss']}s")
        p2.metric("Rec. Max", f"{pit_stats['recommended_max_pit_loss']}s")

    with main_right:
        st.markdown(f"<h2>🏎️ 서킷: {track_name}</h2>", unsafe_allow_html=True)
        path = TRACK_IMAGES_PATHS.get(track_name)
        if path and path.exists(): st.image(str(path), width=TRACK_IMAGE_WIDTHS.get(track_name, 600))

        if start_calc:
            # 기존 계산 로직 및 결과 출력...
            result_df = evaluate_strategies(...)
            # ... 결과 출력 (왼쪽/오른쪽 컬럼 사용하여 결과 배치)

...
def main():
    ...
    with main_left:
        ...
        green_pit_loss = pit_stats['median_pit_loss'] if "자동계산" in use_auto_pit_loss else st.sidebar.number_input("그린 플래그 기준 피트 손실시간(초)", min_value=10.0, max_value=50.0, value=22.0, step=0.5)
        ...
        st.markdown("""<ul style="margin-bottom: 15px; padding-left: 20px; color: #98a2b3; font-size: 0.9rem;"><li><b>실시간 데이터 동기화</b>: 좌측 사이드바 제어창에서 선택된 옵션들은 우측 모니터링 보드와 실시간 연동됩니다.</li><li><b>몬테카를로 시뮬레이션 알고리즘</b>: FastF1 실데이터 모델링을 기반으로 수백 가지 레이스 시나리오를 예측 연산합니다.</li></ul>""", unsafe_allow_html=True)
        st.markdown('<div class="section-label">⚙️ 레이스 컨트롤 전략 보조 가이드</div>', unsafe_allow_html=True)
        st.markdown("""<ul style="margin-bottom: 15px; padding-left: 20px; color: #98a2b3; font-size: 0.9rem;"><li><b>트랙 성향 인자 자동 연산</b>: 서킷별 DRS 효율, Dirty Air 영향성 및 교통(Traffic) 정체 패널티가 상시 반영 중입니다.</li><li><b>실시간 연산 준비</b>: 입력 데이터를 확인하신 후 좌측 사이드바 하단의 주황색 트리거 버튼을 눌러 시뮬레이션을 개시하세요.</li></ul>""", unsafe_allow_html=True)
        ...
        st.dataframe(pd.DataFrame([[t, i['base_offset'], i['deg_per_lap'], i['recommended_stint']] for t, i in tyre_model.items()], columns=['타이어', '성능차(초)', '열화율', '권장 스틴트(랩)']), use_container_width=True, hide_index=True)
    ...
    with main_right:
        ...
        if start_calc:
            ...
            with res_right:
                ...
                if best['stops'] == 0: st.warning("무피트 전략 추천")
                else: st.success(f"추천 피트 랩: **{best['pit_laps']}** (타이어: {best['next_tyres']})")
                
                c1, c2, c3 = st.columns(3)
                c1.metric("예상 순위", f"{best['expected_position']} 위")
                c2.metric("예상 시간", f"{best['expected_finish_time']} 초")
